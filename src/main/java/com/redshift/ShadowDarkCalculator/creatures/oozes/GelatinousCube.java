            ...
            if (target == null) {
                ...
                if (attackHits) {
                    ...
                    if (!target.getStats().constitutionSave(15)) {
                        ...
                        log.info(
                            target.getName() +
                            " is paralyzed and is automatically engulfed by the Gelatinous Cube!"
                        );
                        target.addCondition(new EngulfedInAcidCondition(D8));
                    } else {
                        log.info(
                            target.getName() + " SAVES and is NOT paralyzed!"
                        );
                        ...
                        if (!target.getStats().strengthSave(12)) {
                            log.info(
                                target.getName() +
                                " fails a STR save and is engulfed by the Gelatinous Cube!"
                            );
                            target.addCondition(
                                new EngulfedInAcidCondition(D8)
                            );
                        }
                    }
                }
